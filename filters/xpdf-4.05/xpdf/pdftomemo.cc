...
static GBool memoMeta = gFalse; // MEMO Option
...
int main(int argc, char *argv[]) {
  ...
  if (!ok || argc < 2 || argc > 3 || printVersion || printHelp) {
    fprintf(stderr, "pdftomemo based upon pdftotext version %s [www.xpdfreader.com]\n", xpdfVersion);
    fprintf(stderr, "%s\n", xpdfCopyright);
    if (!printVersion) {
      printUsage("pdftomemo", "<PDF-file> [<text-file>]", argDesc);
    }
    goto err0;
  }
  fileName = argv[1];
