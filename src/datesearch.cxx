...
PIRSET INDEX::DoDateSearch(const STRING& QueryTerm, const STRING& FieldName, 
			 INT4 Relation, INT4 Structure) 
{
  return DoDateSearch(QueryTerm, FieldName, Relation, Structure, LOOSE_MATCH);
}
...
PIRSET INDEX::DateRangeSearch(const STRING& QueryTerm, const STRING& FieldName, 
...
//cerr << YMD << " returned " << endl;
  ...
  if (((Today.Equals(QueryDate)) && (FindBlock == END_BLOCK) 
    ...
    p_today = DateSearch(DAY_UPPER, FieldName, ZRelGT, SEARCH_END);
		      ...
		      IntBlock FindBlock, bool fIncludeEndpoint)
{
  ...
  if (pirset->GetTotalEntries() > 0) {
    ...
    // value.
    LowerBound = DateSearch(MONTH_LOWER, FieldName, ZRelGT, FindBlock);
		       ...
		       IntBlock FindBlock, bool fIncludeEndpoint)
{
  ...
  if (pirset->GetTotalEntries() > 0) {
    ...
    // lower bound value.
    LowerBound = DateSearch(DAY_LOWER, FieldName, ZRelGT, FindBlock);
		     ...
		     IntBlock FindBlock, bool fIncludeEndpoint)
{
  ...
  if (fIncludeEndpoint)
    pirset = DateSearch(fValue, FieldName, ZRelGE, FindBlock);
  else
    pirset = DateSearch(fValue, FieldName, ZRelGT, FindBlock);
  ...
  if (pirset->GetTotalEntries() > 0) {
	...
	|| ((Today.Equals(DateYMD)) && (FindBlock == END_BLOCK))) {
      if (fIncludeEndpoint)
	UpperBound = DateSearch(DAY_UPPER, FieldName, ZRelLT, FindBlock);
      else
	UpperBound = DateSearch(DAY_UPPER, FieldName, ZRelLE, FindBlock);
		     ...
		     IntBlock FindBlock, bool fIncludeEndpoint)
{
  ...
  if (pirset->GetTotalEntries() > 0) {
    ...
    // upper bound value.
    UpperBound = DateSearch(MONTH_UPPER, FieldName, ZRelLT, FindBlock);
		     ...
		     IntBlock FindBlock, bool fIncludeEndpoint)
{
  ...
  if (pirset->GetTotalEntries() > 0) {
    ...
    // upper bound value.
    UpperBound = DateSearch(YEAR_UPPER, FieldName, ZRelLT, FindBlock);
...
size_t INDEX::DateRangeScan(STRLIST *Strlist, const STRING& FieldName, size_t Start, INT TotalRequested)
{
  FIELDTYPE    FieldType = Parent->GetFieldType(FieldName);
  STRING       Fn;
  INTERVALLIST List;
  ...
  for (size_t i=0; i<count; i++)
   {
     ...
     start = List.GetStartValue(i);
     end   = List.GetEndValue(i);
     if (date != oldDate)
       {
          Strlist->AddEntry(date.LCdate());
          oldDate = date;
          total++;
