...
#define OUTPUT_BLOCK_SIZE  4	/* size in 32-bit words of an output block from the hash routine */
#endif
...
/* The standard S-Box  */
static const UINT4 standardSBoxes[MAX_SBOX_COUNT][256]= {
 {  /* Start of S Box 0  */
 ...
 * message and aborts
 */
static void ErrAbort (const char *s)
{
  fprintf (stderr, "%s\n", s);
  exit (1);
}
...
#if IS_BIG_ENDIAN
/*
 * The following routine converts a byte array to an array of UINT4
 ...
 */
static void ConvertChunkOfBytes (const char *charBuffer, UINT4 *wordBuffer)
{
	int     i;
	UINT4 t0, t1, t2, t3;
 ...
 */
void HashN (UINT4 *output, const UINT4 *input)
{
	static int shiftTable[4] = {16, 8, 16, 24};
	/* the array of data being hashed  */
	UINT4 block[INPUT_BLOCK_SIZE];
	UINT4 SBoxEntry;	/* just a temporary */
	int     shift;
	int     i;
	int     index;
	int     next, last;
 ...
 * possible, it requires great caution.
 */
void Hash512 (UINT4 *output, const UINT4 *input)
{
	static int shiftTable[4] = {16, 8, 16, 24};
	/* the array of data being hashed  */
	UINT4	SBE;	/* just a temporary */
	int     shift, leftShift;
	int     index;
	int     byteInWord;
	const UINT4	*SBox0;
	const UINT4	*SBox1;
	UINT4	B00,B01,B02,B03,B04,B05,B06,B07,B08,B09,B10,B11,B12,B13,B14,B15;
 ...
 */
static void Increment64BitCounter (UINT4 *counter, long int increment)
{
	UINT4 maxInt = 0xffffffffL;
...
{
  return SnefruHash(Input.c_str(), Input.GetLength());
}

STRING SnefruHash(const char *CString)
{
  return SnefruHash(CString, CString ? strlen(CString) : 0);
}
...
STRING SnefruHash(const char *ptr, size_t length)
{
  ...
  size_t i;
  char   tmp[32];
  UINT4 hash[INPUT_BLOCK_SIZE];
  UINT4 bitCount[2];     /* the 64-bit count of the number of bits in the input */
  long int byteCount;     /* the count of the number of bytes we just read */
        ...
        bitCount[1] = 0;
	/* initialize hash  */
 	memset(hash, 0, INPUT_BLOCK_SIZE*sizeof(UINT4));
        /*
...
Contact edz@nonmonotonic.com!!", byteCount, CHUNK_SIZE*4);
	  ...
	  for (size_t j = byteCount; j < CHUNK_SIZE*4; j++)
	    charBuf[j] = 0;
        /*
         ...
         */
#if IS_BIG_ENDIAN
        ConvertChunkOfBytes (charBuf, hash+OUTPUT_BLOCK_SIZE);
#endif
	...
	/* hash in the block we just read  */
	 if (byteCount > 0) {
	  ...
	  break;
        } while (i < length);  /* end of while */
        /*
         * Put the 64-bit bit-count into the final 64-bits of the block about
         * to be hashed
         */
        hash[INPUT_BLOCK_SIZE - 2] = bitCount[0];       /* upper 32 bits of count */
        hash[INPUT_BLOCK_SIZE - 1] = bitCount[1];       /* lower 32 bits of count */
        /* Final hash down.  */
        Hash512 (hash, hash);
...
static STRING SimpleHash(const char *key, size_t length)
{
  unsigned int h = 0;
  ...
  return STRING().form("%x%x", h, length);
}

STRING OneWayHash(const char *ptr, size_t length)
{
  if (length < 512)
    return SimpleHash(ptr, length);
  return SnefruHash(ptr, length);
}


STRING OneWayHash(const STRING& Input)
{
  return OneWayHash(Input.c_str(), Input.Length());
}

STRING OneWayHash(const char *CString)
{
  return OneWayHash(CString, CString ? strlen(CString) : 0);
}


#if 0

UINT4 one_at_a_time(unsigned char *key, size_t len)
{
  UINT4 hash = 0;
