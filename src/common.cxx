...
FILE *fmemopen(void *buf, size_t len, const char *type)
{
	...
	if (!GetTempFileNameA(tp, tfname, 0, fn))
		return NULL;
	retner = _sopen_s(pfd, fn,
		 _O_CREAT | _O_SHORT_LIVED | _O_TEMPORARY | _O_RDWR | _O_BINARY | _O_NOINHERIT, _SH_DENYRW, _S_IREAD | _S_IWRITE);
	if (retner != 0)
		return NULL;
	if (fd == -1)
		return NULL;
...
int encode58(const unsigned char *source, int len, unsigned char result[], int reslen)
{
...
int decode58( const unsigned char *str, int len, unsigned char *result)
{
    result[0] = 0;
    int resultlen = 1;
    for (int i = 0; i < len; i++) {
